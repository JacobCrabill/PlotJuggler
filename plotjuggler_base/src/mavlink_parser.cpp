...
{
  return uint32_t(buf[0] & 0xff) | uint32_t(buf[1] & 0xff) << 8 | uint32_t(buf[2] & 0xff) << 16 |
         uint32_t(buf[3] & 0xff) << 24;
}

int32_t readInt32(uint8_t* buf)
{
  return int32_t(buf[0] & 0xff) | int32_t(buf[1] & 0xff) << 8 | int32_t(buf[2] & 0xff) << 16 |
         int32_t(buf[3] & 0xff) << 24;
}

uint64_t readUint64(uint8_t* buf)
{
  return uint64_t(buf[0] & 0xff) | uint64_t(buf[1] & 0xff) << 8 | uint64_t(buf[2] & 0xff) << 16 |
         uint64_t(buf[3] & 0xff) << 24 | uint64_t(buf[4] & 0xff) << 32 | uint64_t(buf[5] & 0xff) << 40 |
         uint64_t(buf[6] & 0xff) << 48 | uint64_t(buf[7] & 0xff) << 56;
}

int64_t readInt64(uint8_t* buf)
{
  return int64_t(buf[0] & 0xff) | int64_t(buf[1] & 0xff) << 8 | int64_t(buf[2] & 0xff) << 16 |
         int64_t(buf[3] & 0xff) << 24 | int64_t(buf[4] & 0xff) << 32 | int64_t(buf[5] & 0xff) << 40 |
         int64_t(buf[6] & 0xff) << 48 | int64_t(buf[7] & 0xff) << 56;
}

bool MAVLink_Parser::parseMessage(const MessageRef msg, double& timestamp)
{
  bool ret = false;
